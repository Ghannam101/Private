// ThumbHash — a very compact (~25-byte) structure-preserving image placeholder.
// Verbatim canonical Swift reference implementation by Evan Wallace. MIT License.
// Source: https://github.com/evanw/thumbhash/blob/main/swift/ThumbHash.swift
// We use `imageToThumbHash(image:)` to encode a poster/logo once (off-main, cached
// in GRDB) and `thumbHashToImage(hash:)` to paint an instant blurred placeholder
// on the next cold start, before the real image finishes loading. DO NOT edit the
// numeric code below — it must stay byte-identical to the reference to stay correct.
